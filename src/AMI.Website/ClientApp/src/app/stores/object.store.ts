...
export class ObjectStore {
  ...
  private initGateway(): void {
    this.gateway.on(EventType[EventType.TaskCreated], (data: TaskModel) => {
      this.updateLatestTask(TaskModel.fromJS(data));
    });
    this.gateway.on(EventType[EventType.TaskUpdated], (data: TaskModel) => {
      this.updateLatestTask(TaskModel.fromJS(data));
    });
    this.gateway.on(EventType[EventType.ObjectCreated], (data: ObjectModel) => {
      if (data && data.id) {
        this.addItem(ObjectModel.fromJS(data) as ObjectModelExtended);
      }
    });
    this.gateway.on(EventType[EventType.ObjectDeleted], (data: ObjectModel) => {
      if (data && data.id) {
